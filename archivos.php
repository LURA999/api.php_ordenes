...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
     if(isset($_FILES['info'])){
        ...
        if(move_uploaded_file($_FILES['info']['tmp_name'], $directorio_final)){
            $sql = $dbConn->prepare("CALL sp_insertarDocumentosOrden(".$_GET['cve_comentario'].",'".$imagen_nombre."')");
            $sql->execute();
            $data = array(
                'status' => 'success',
                'code' => 200,
...
if($_SERVER['REQUEST_METHOD'] == 'GET'){ 
        ...
        $sql = $dbConn->prepare("CALL sp_selectDocumentosOrden(".$_GET['cve_comentario'].")");
        $sql->execute();
